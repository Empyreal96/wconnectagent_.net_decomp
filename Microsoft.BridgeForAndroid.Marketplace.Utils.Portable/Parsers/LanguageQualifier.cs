...
// Type: Microsoft.Arcadia.Marketplace.Utils.Parsers.LanguageQualifier
  ...
  public static class LanguageQualifier
  {
    ...
    public static bool IsValidLanguageQualifier(string languageQualifier)
    {
      if (string.IsNullOrWhiteSpace(languageQualifier))
        throw new ArgumentException("A valid language qualifier must be provided.", nameof (languageQualifier));
      if (!LanguageQualifier.AllSupportedLanguageQualifiers.Contains(languageQualifier.ToUpper()))
      {
        LoggerCore.Log("{0} is not a whitelisted language qualifier.", (object) languageQualifier);
        return false;
      }
      try
      {
        CultureInfo cultureInfo = new CultureInfo(languageQualifier);
      }

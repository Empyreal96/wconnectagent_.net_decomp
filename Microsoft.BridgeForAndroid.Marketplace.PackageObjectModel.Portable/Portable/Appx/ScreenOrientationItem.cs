// Decompiled with JetBrains decompiler
// Type: Microsoft.Arcadia.Marketplace.PackageObjectModel.Portable.Appx.ScreenOrientationItem
// Assembly: Microsoft.BridgeForAndroid.Marketplace.PackageObjectModel.Portable, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: C5DFC509-F9BD-4763-8503-317A9C276F67
// Assembly location: .\\AowDebugger\Agent\Microsoft.BridgeForAndroid.Marketplace.PackageObjectModel.Portable.dll

using System.Collections.Generic;

namespace Microsoft.Arcadia.Marketplace.PackageObjectModel.Portable.Appx
{
  public sealed class ScreenOrientationItem
  {
    public ScreenOrientationItem(
      AppxScreenOrientationCategory screenOrientationCategory,
      HashSet<AppxScreenOrientationType> possibleScreenOrientationTypes)
    {
      this.ScreenOrientationCategory = screenOrientationCategory;
      this.PossibleScreenOrientationTypes = possibleScreenOrientationTypes;
    }

    public AppxScreenOrientationCategory ScreenOrientationCategory { get; private set; }

    public HashSet<AppxScreenOrientationType> PossibleScreenOrientationTypes { get; private set; }
  }
}

...
// Type: Microsoft.Arcadia.Debugging.AdbProtocol.Portable.AdbFileSyncReceivePacket
...
namespace Microsoft.Arcadia.Debugging.AdbProtocol.Portable
{
  ...
  {
    public AdbFileSyncReceivePacket(string deviceFilePath) => this.DeviceFilePath = deviceFilePath;

    public string DeviceFilePath { get; private set; }

    public static async Task<AdbFileSyncReceivePacket> ReadBodyAsync(
      IStreamReader stream)
    {
      ...
      if (bytes.HasValue)
      {
        ...
        if ((nullable.GetValueOrDefault() != 0U ? 0 : (nullable.HasValue ? 1 : 0)) == 0)
        {
          ...
          return data != null ? new AdbFileSyncReceivePacket(data) : (AdbFileSyncReceivePacket) null;
        }
      }
      return (AdbFileSyncReceivePacket) null;
    }
